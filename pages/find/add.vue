<template>
	<view class="content">
		...
		<view style="font-size: 28rpx;margin-top: 30rpx;">品种</view>
		<view class="breedView"><input type="text" v-model="petInfo.breed" maxlength="8" placeholder="请输入宠物品种" /></view>
		...
		<!-- <view style="font-size: 28rpx;margin-top: 30rpx;">具体走失区域</view> -->
		<view class="addressView"><input type="text" v-model="petInfo.address" placeholder="请输入具体走失区域" /></view>

		<view style="font-size: 28rpx;margin-top: 30rpx;">详情</view>
		<view class="contentView">
			<textarea maxlength="200" v-model="petInfo.content" placeholder="请输入宠物特征/宠物描述/丢失原因...(最多200字)"></textarea>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			addresspick(obj) {
				let addr = obj.province.AreaName +" "+ obj.area.AreaName +" "+ obj.city.AreaName
				this.city = addr
			...
			submit() {
				...
				http.addPetFind(this.petInfo).then(res => {
					if(res.success) {
						uni.showToast({
							title: '发布成功',
							icon: 'success',
			...
			paramCheck() {
				...
				if(this.petInfo.nickname == null || this.petInfo.breed == "") {
					uni.showToast({title: '请填写昵称',icon: 'none'});
					return false
				}
				if(this.petInfo.breed == null || this.petInfo.nickname == "") {
				...
				if(this.petInfo.address == null || this.petInfo.address == "") {
					uni.showToast({title: '请填写具体走失区域',icon: 'none'});
					return false
				}
